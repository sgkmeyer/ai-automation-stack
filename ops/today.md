...
- [x] Implement unified recall router v1 first slice across memory, transcripts, registry, and context (2026-03-21)
...
- **Hybrid wiki lane (live Phase 1 slice)** — deployed on 2026-04-10/11 (`06f54f1`, hotfix `10ef2d8` on prod/dev). `./bin/wiki --help`, `health`, `search`, `lint`, `proposals`, `propose`, and `review` are live in the Openclaw workspace; proposal creation now succeeds after the frontmatter serialization hotfix, approved proposals land in the VPS outbox, synthesis-style router queries resolve `primary_lane=wiki` with live-lane fallback disclosure, and actor metadata is preserved on proposal records for TARS/Openclaw and other agent clients.
- **Canonical Obsidian wiki hydration started** — on 2026-04-10/11 the local vault at `/Users/sgkmeyer/vaults/second-brain` was bootstrapped with control pages plus starter source/synthesis notes, approved VPS proposals were synced into `_review/wiki-proposals`, one approved smoke-test page was promoted into the canonical vault, and the canonical wiki was mirrored into the live runtime wiki mount. `./bin/query-wiki "hybrid memory" --limit 5` now returns hydrated wiki pages on production, and `./bin/recall-unified --query "Give me the deeper picture on the hybrid memory stack"` now answers from the wiki lane without fallback.
