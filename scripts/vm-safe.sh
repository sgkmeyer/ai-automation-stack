...
case "${action}" in
  ...
  backup)
    run_vm_cmd \
      ...
      "set -euo pipefail; cd /home/ubuntu; tar_rc=0; sudo tar --ignore-failed-read --warning=no-file-changed --warning=no-file-removed -hczf automation-full-\$(date +%F-%H%M).tar.gz automation || tar_rc=\$?; if [ \"\${tar_rc}\" -gt 1 ]; then exit \"\${tar_rc}\"; fi; docker run --rm -v automation_db_storage:/data -v /home/ubuntu:/backup busybox tar czf /backup/automation-db-\$(date +%F-%H%M).tar.gz /data; ls -lh /home/ubuntu/automation-full-*.tar.gz /home/ubuntu/automation-db-*.tar.gz | tail -n 6"
    ;;
  dr-backup)
    ts="$(date +%F-%H%M%S)"
    ...
    # shellcheck disable=SC2029
    ssh "${VM_HOST}" "set -euo pipefail; cd /home/ubuntu; tar_rc=0; sudo tar --ignore-failed-read --warning=no-file-changed --warning=no-file-removed -hczf ${remote_cfg} automation || tar_rc=\$?; if [ \"\${tar_rc}\" -gt 1 ]; then exit \"\${tar_rc}\"; fi; docker run --rm -v automation_db_storage:/data -v /home/ubuntu:/backup busybox tar czf /backup/$(basename "${remote_db}") /data; ls -lh ${remote_cfg} ${remote_db}; sha256sum ${remote_cfg} ${remote_db}"
